...
def online_learning(model, data_loader, optimizer, criterion, device, online_epochs):
    model.train()
    losses = []
    for batch_x, batch_y, batch_x_mark in data_loader:
        ...
        # Evaluate before online learning
        with torch.no_grad():
            outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
            outputs = outputs[:, :, 0].unsqueeze(-1)
            initial_loss = criterion(outputs, batch_y).item()
        ...
        # Evaluate after online learning
        with torch.no_grad():
            outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
            outputs = outputs[:, :, 0].unsqueeze(-1)
            final_loss = criterion(outputs, batch_y).item()
        
        losses.append([initial_loss, final_loss])
    
    return model, np.mean(losses, axis=0)
...
def main():
    ...
    y_val_tensor = torch.FloatTensor(y_val).unsqueeze(-1).unsqueeze(-1)  # Shape: [samples, 1, 1]
    ...
    val_mark = torch.arange(config.seq_len).repeat(X_val.shape[0], 1)
    val_mark = val_mark.unsqueeze(-1).repeat(1, 1, 4).float()
    ...
    val_dataset = TensorDataset(X_val_tensor, y_val_tensor, val_mark)
    ...
    print("Training finished.")
     ...
     # Save the model after 6 epochs
    torch.save(model.state_dict(), 'timesnet_model_6epochs.pth')
    print("Model saved after 6 epochs.")

    # Load the saved model
    model.load_state_dict(torch.load('timesnet_model_6epochs.pth'))
    print("Model loaded for online learning.")

    # Perform online learning
    online_epochs = 3
    online_optimizer = optim.Adam(model.parameters(), lr=2e-4, weight_decay=1e-4)
    model, online_losses = online_learning(model, test_loader, online_optimizer, criterion, device, online_epochs)
    
    print(f"Online Learning - Initial Loss: {online_losses[0]:.4f}, Final Loss: {online_losses[1]:.4f}")
    ...
    print("Final Evaluation after Online Learning:")
    print(f"Test Loss: {test_loss:.4f}, MAE: {mae:.4f}, RMSE: {rmse:.4f}, R^2: {r2:.4f}")
