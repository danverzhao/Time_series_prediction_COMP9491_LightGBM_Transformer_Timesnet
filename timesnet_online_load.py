...
def online_train(model, batch_x, batch_y, batch_x_mark, val_loader, optimizer, criterion, device, epoch):
...
def main():
    ...
    val_data = val_loader
    online_val_losses = []
    for X_v, y_v, X_mark in tqdm(val_data, "Evaluating validation loss"):

        mae, rmse, r2 = online_train(model, X_v, y_v, X_mark, val_loader, optimizer, criterion, device, ONLINE_EPOCHS)
    ...
    online_test_losses = []
    for X_v, y_v, X_mark in tqdm(test_data, "Evaluating test loss"):
